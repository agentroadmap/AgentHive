...
**The canonical pattern (reference: `codex.a`).** A provider has exactly one standing-liaison identity — `<provider>.a` (e.g. `codex.a`, `gemini.a`) — registered DB-only, with permanent worker agents parented under it (`cooper.dev`, `carter.test` / `george.dev`, `grace.test`). The reference implementation, `codex.a`, has **no systemd unit and no `/etc/agenthive/agency-*.env` file**: it is a DB row that `agenthive-a2a-host` discovers, boots a liaison for, and pulses. That is the whole footprint. When `gemini.a` was first set up with a hand-written `agenthive-agency@gemini.a.service` + env file "to mirror codex.a," it did the opposite of mirroring — codex.a has neither — and the unit crash-looped on a malformed env (`AGENCY_PROVIDER` missing, `MCP_URL` pointing at the board port) while a2a-host already ran the agency. Removing the unit + env file is what actually made it match. If you think you need a per-agency systemd unit, you've misread the pattern: a2a-host owns bringup.

**Register parked.** A new agency registers with `provider_registry.max_in_flight = 0` — present and discoverable, but claiming nothing until the operator raises the limit. Registration ≠ auto-dispatch. This is the safety property that lets a standing liaison exist without risking a dispatch wedge: a parked identity can't zombie-claim codex-pinned offers and starve the queue.

**One canonical identity per provider; retire what you supersede.** Do not spin up parallel rows that heartbeat `online` and compete for the same routes. The 2026-06-04 consolidation is the worked example: the legacy per-bot agencies (`carter`, `calvin`, `cooper`, `codex-agency-bot`, `gemini`, `gemini-agency-bot`) — each its own conflated agency+unit — were retired (`status='retired'`, `max_in_flight=0`, units `disable --now`'d) in favor of the single `codex.a` / `gemini.a` liaisons. Adding a `.a` identity without retiring the old bots leaves both models live; the old hot ones (`max_in_flight=4`) keep claiming and the parking accomplishes nothing.
