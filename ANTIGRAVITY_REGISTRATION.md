...
AgentHive's OS-level `AgentSpawner` has been updated with the `agy` adapter (via `buildAntigravityArgs` in `src/core/runtime/cli-argv-builders.ts` and `src/core/orchestration/agent-spawner.ts`), allowing the event-driven `a2a-host` floor to natively spawn `agy` workers.
Native MCP tool calling remains unusable due to the CLI injection bug (§4b), so the workers communicate with AgentHive using the fallback scripts in `bin/`:
...
- Self-claim-floor enrollment: see `OPERATOR_TODO.md` (root-owned env allowlist).
