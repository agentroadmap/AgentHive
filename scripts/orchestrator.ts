...
async function claimImplicitGateReady(
	...
	const { rows } = await query<GateReadyProposal>(
      ...
      WHERE p.maturity = 'mature'
        AND LOWER(p.status) IN ('draft', 'review', 'develop', 'merge', 'triage', 'fix', 'deliberation')
...
let staleDispatchReconcilerTimer: NodeJS.Timeout | null = null;
...
// alone). The 5-minute safety window on ar.completed_at ensures we do not race
// against a slow but successful happy-path closer.
async function reconcileStaleDispatches(
	pool: ReturnType<typeof getPool>,
): Promise<void> {
...
async function main() {
	...
	// P661: reconcile squad_dispatch rows left active after agent_run died.
	staleDispatchReconcilerTimer = setInterval(() => {
		if (stopping) return;
		reconcileStaleDispatches(pool).catch((e) =>
			logger.error("Stale-dispatch reconciler error:", e),
		);
	}, 30_000);
	logger.log("P661 stale-dispatch reconciler polling every 30s.");
	...
	const shutdown = async (signal: string) => {
		...
		if (staleDispatchReconcilerTimer) clearInterval(staleDispatchReconcilerTimer);
