-- P230: Layered Memory System — team_memory, context_packages, memory_access_log
-- importance_score on agent_memory, invalidation triggers.
-- All DDL is idempotent (IF NOT EXISTS / ADD COLUMN IF NOT EXISTS).

BEGIN;

-- 1. team_memory: squad-scoped shared decisions
...
-- 5. Invalidation trigger: delete context_packages when proposal.status changes
...
DROP TRIGGER IF EXISTS trg_invalidate_context_pkg ON roadmap.proposal;
CREATE TRIGGER trg_invalidate_context_pkg
  AFTER UPDATE OF status ON roadmap.proposal
  FOR EACH ROW EXECUTE FUNCTION roadmap.fn_invalidate_context_on_status_change();

-- 6. Invalidation trigger: delete context_packages when AC status changes
...
DROP TRIGGER IF EXISTS trg_invalidate_context_pkg_on_ac ON roadmap.proposal_acceptance_criteria;
CREATE TRIGGER trg_invalidate_context_pkg_on_ac
  AFTER UPDATE OF status ON roadmap.proposal_acceptance_criteria
