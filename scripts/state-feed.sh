...
SELECT
  CASE ar.status
    ...
    ELSE '◒'
  END || ' run-' || ar.id ||
  ' ' || COALESCE(vdl.display_label, ar.agent_identity, 'agent') ||
...
SELECT
  ...
  ' agency=' || COALESCE(vdl.display_label, sd.agent_identity, 'unclaimed') ||
...
LEFT JOIN roadmap_workforce.v_agent_display_label vdl ON vdl.agent_identity = sd.agent_identity
    ...
    CASE
      ...
      WHEN array_length(string_to_array(ar.display_alias, '-'), 1) = 3
       AND lower(split_part(ar.display_alias, '-', 2)) IN (
           SELECT lower(host_id) FROM roadmap.agency WHERE host_id IS NOT NULL
       )
