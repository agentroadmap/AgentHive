...
CREATE TABLE IF NOT EXISTS core.runtime_flag (
  ...
  category          TEXT         NOT NULL DEFAULT 'general'
                                CHECK (category IN (
                                  'database', 'connection_pool', 'vault_secret',
                                  'mcp_endpoint', 'orchestrator', 'dispatch',
                                  'liaison', 'pause_backoff', 'provider_quota',
                                  'adaptive_matcher', 'gate_governance', 'multi_tenant',
                                  'model_routing', 'audit', 'ui_ux', 'diagnostic',
                                  'general'
                                )),
  description       TEXT,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- core.config_mutation_log is defined in 001b-core-post-identity.sql.
-- It has a FK to control_identity.principal, so it must be created AFTER
-- 002-identity.sql. Moving it here would break clean provisioning.
-- ─────────────────────────────────────────────────────────────────────────────
