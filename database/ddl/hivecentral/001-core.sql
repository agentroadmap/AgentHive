...
-- Minimum PostgreSQL version: 14 (required for CREATE OR REPLACE TRIGGER).
-- For PG ≤13, replace CREATE OR REPLACE TRIGGER with:
--   DROP TRIGGER IF EXISTS <name> ON <table>;
--   CREATE TRIGGER <name> ...;
-- ============================================================
-- Target DB: hiveCentral
-- Owner: agenthive_admin
-- Roles granted: see grant matrix in database/ddl/hivecentral/README.md
-- ============================================================
...
-- ============================================================
-- Shared trigger function: advance updated_at on catalog rows
-- Defined once here; attached to all four catalog tables below.
-- NOT attached to service_heartbeat (operational table, hygiene exemption).
-- ============================================================
...
-- Hygiene-field exemption: operational/telemetry table — no ownership concept,
-- no lifecycle (rows replaced via ON CONFLICT UPDATE, never deprecated/retired),
-- and high write volume makes unnecessary hygiene columns costly. See README §hygiene.
...
-- Grants
-- ============================================================
-- Roles are created in 000-roles.sql (superuser context, run on postgres DB first).
-- IF EXISTS guards: safe to run before role creation (grants silently skipped).

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'agenthive_admin') THEN
    GRANT ALL ON SCHEMA core TO agenthive_admin;
    GRANT ALL ON ALL TABLES IN SCHEMA core TO agenthive_admin;
    GRANT ALL ON ALL SEQUENCES IN SCHEMA core TO agenthive_admin;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'agenthive_orchestrator') THEN
    ...
    -- orchestrator: rw on runtime_flag and service_heartbeat; ro on host/os_user/installation.
    -- MUST NOT hold INSERT or UPDATE on core.host or core.os_user — catalog writes belong to
    -- provisioning workflows (P601), not the orchestrator. Granting write access to these two
    -- tables is a least-privilege violation in a security-critical area.
...
-- ============================================================
-- Inserted only on fresh DB; WHERE NOT EXISTS prevents double-seeding.
-- Re-applying this file is always safe.
