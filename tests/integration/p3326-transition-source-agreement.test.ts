...
// Self-healing sweep: the per-test cleanup() runs in a `finally`, but a hard
// interruption (SIGTERM/timeout) or crash bypasses it, leaving committed
// `P3326-test-*` proposals in the live queue where the orchestrator then picks
// them up and spawns real agent runs. Purging by tag at suite start/end bounds
// the damage to a single interrupted run — the next run cleans up after it.
const TEST_DISPLAY_PREFIX = "P3326-test-%";
async function purgeTestLeftovers(pool: Pool) {
	...
	describe("AC-1: gate_decision D2 advance resolves DEVELOP (not COMPLETE)", () => {
		it("picks DEVELOP over COMPLETE via stage_order for Standard RFC in REVIEW", async () => {
            ...
            LIMIT 1`,
				...
				assert.ok(
					fwd.length > 0,
					"Should find a forward transition from REVIEW",
				);
				assert.equal(
