...
describe("ClaudeCliBuilder", () => {
  ...
  it("buildArgv() — with modelOverride inserts --model flag", () => {
    const argv = builder.buildArgv({ task: "do work", modelOverride: "claude-opus-4-7" });
  ...
  it("buildEnv() — injects ANTHROPIC_API_KEY from vault", () => {
    const env = builder.buildEnv({ homeDir, apiKeyVault: { ANTHROPIC_API_KEY: "sk-test-key" } });
  ...
  it("buildCommandSpec() — argv starts with claude --print --model", () => {
    ...
    assert.ok(spec.argv.includes("claude-sonnet-4-6"), "must include model name");
...
describe("CodexCliBuilder", () => {
  ...
  it("buildArgv() — includes 'exec' subcommand and bypass flag", () => {
    ...
    assert.ok(argv.includes("--dangerously-bypass-approvals-and-sandbox"), "must include bypass flag");
  ...
  it("buildEnv() — injects OPENAI_API_KEY from vault", () => {
    const env = builder.buildEnv({ homeDir, apiKeyVault: { OPENAI_API_KEY: "sk-openai-key" } });
  ...
  it("buildCommandSpec() — non-default baseUrl sets OPENAI_BASE_URL", () => {
    ...
    assert.equal(spec.env.OPENAI_BASE_URL, "https://custom-openai.example.com/v1");
  });
});
...
describe("HermesCliBuilder", () => {
  ...
  it("buildEnv() — injects NOUS_API_KEY and maps to OPENAI_API_KEY", () => {
    const env = builder.buildEnv({ homeDir, apiKeyVault: { NOUS_API_KEY: "nous-key-123" } });
