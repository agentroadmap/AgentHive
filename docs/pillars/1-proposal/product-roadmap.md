...
| P377 | Sync proposal_valid_transitions from workflow_transitions — single source of truth for transition validation | 3/3 |
...
| P407 | Orchestrator retry cascade: 10,632 SPAWN_POLICY_VIOLATION + 38 AGENT_DEAD escalations from missing backoff/dedup | 0/5 |
...
| P434 | Provider Route and Budget Governance: token plans, API key plans, hierarchical budgets, context policy | 3/3 |
| P435 | Control Panel Observability: web/TUI/mobile feeds with causal IDs and stop scopes | 9/9 |
| P436 | Schema Reconciliation for Control Plane: resolve drift, classify proposal/workflow as control-plane | 3/3 |
...
| P447 | Cubic Worktree Path Normalization: canonical /data/code/worktree/<name> root, repair legacy rows | 0/4 |
| P448 | Multi-tenant path/user hardcoding cascade — provider switch is destructive | 5/5 |
| P449 | MCP URL and daemon URL hardcoded across 30+ files — blocks multi-host MCP and parallel agencies | 0/4 |
...
| P454 | Extract scan-hardcoding to standalone @agenthive/scan package with multi-pack rule library and binary distribution | 8/8 |
| P455 | hive CLI: structured replacement for roadmap CLI — daily design + operations surface, AI-agent-friendly, control-plane-aware | 3/3 |
| P456 | MCP handler argument-naming inconsistency — every action uses different camelCase/snake_case for the same concept | 7/7 |
| P457 | proposal_discussions.context_prefix CHECK rejects gate-decision/ship-verification/handoff prefixes that CONVENTIONS.md §4a tells agents to use | 0/3 |
...
| P460 | fn_spawn_workflow trigger doesn't always create roadmap.workflows row — P448 stuck unable to transition | 0/7 |
...
| P465 | Subscription-aware claim policy in liaisons — 5h/daily/weekly/monthly window detection and proactive refusal | 11/11 |
...
| P673 | Architecture-type proposal lifecycle: POC, tech-stack / model selection, tuning — unify operator-in-the-loop design phases | 0/10 |
...
| P754 | A7: decommission agenthive-gate-pipeline.service + delete pipeline-cron.ts + start-gate-pipeline.ts + CONVENTIONS §6.0b | 0/5 |
...
| P774 | P706-C1: Migration — workflow vocab unification (Hotfix→3-stage, drop Quick Fix, add obsoleted_reason, rewrite triggers) | 5/7 |
...
| P777 | P706-C4: TUI Board — force Workflow filter + dynamic columns + filter row redesign matching web | 7/7 |
| P778 | P706-C5: Gate-evaluator closure verdicts → maturity=obsolete + obsoleted_reason (no terminal status flip) | 10/10 |
...
| P854 | P844 pool gate gap: propagate _auth principal into agentContextStorage inside callTool() + add bearer auth to MCP server HTTP transport | 1/5 |
...
| P994 | P993 Phase 2 — Task Protocol Completeness (task_ack fields, AC verification on complete, watchdog wiring, E2E test) | 4/4 |
...
| P1072 | Vault adapter DB integration: wire control_credential.vault_provider + credential into src/shared/vault/index.ts | 21/26 |
...
| P1367 | P996 amendment: hyphen separator + LLM variant slot for agency aliases (claude-a, claude-mimo-a, codex-a, …) | 8/8 |
...
| P1387 | MCP proposal-surface DX: teach future agents which write action is visible, fix submit_review param asymmetry, surface claim-first workflow | 5/5 |
...
| P1409 | hotfix: MCP proposal_reviews.create silently drops is_blocking=true (sibling of P1389 silent-input-drop) | 4/4 |
| P1411 | Audit raw SELECTs against roadmap_proposal.proposal for gate_scanner_paused filter — close the bug-class | 7/7 |
| P1431 | hotfix: liaison reply messageType uses 'status' (rejected by CHECK constraint) + crash kills whole liaison | 0/4 |
...
| P1456 | P1350-E: Session-instance identities for concurrent interactive CLI sessions — addressable A2A endpoints under one principal | 17/18 |
...
| P1859 | Provider usage probe — the missing P1004 writer (subscription /usage endpoint, per-(OS-user,provider) credential) | 8/8 |
...
| P2322 | Insulate live services from agent checkout contamination (dedicated deploy checkout + isolation enforcement) | 5/5 |
| P2323 | Stop test fixtures polluting the live DB: test isolation + agent/agency reaper + heartbeat-gated status | 5/5 |
...
| P2404 | Reconcile liveness poke with cold-wake liaisons: fresh heartbeat must override a stale timed-out poke in v_agency_status | 3/5 |
| P2408 | Antigravity (agy) spawn adapter — buildAntigravityArgs + agent_cli dispatch so the a2a-host floor can spawn agy workers | 5/7 |
| P2496 | Offer-generator terminal-state hardening — COMPLETE proposals post zero offers | 6/6 |
| P2709 | Bulk reeval job flaps maturity new<->mature across all gate-eligible proposals (~5-7 min cycle) | 5/5 |
| P2754 | P150 (and possibly others) cannot transition DEVELOP→MERGE — 'not allowed for this workflow' despite valid Standard RFC row | 5/5 |
| P2755 | gate_decision MCP shortcut never advances status — handler writes to_state=from_state, defeating fn_apply_gate_advance | 5/5 |
| P2756 | Workflow drift root cause + guard: proposal creation seeds workflows-table template disagreeing with proposal_type_config (60 proposals affected) | 8/8 |
| P2969 | fn_apply_gate_advance: attribute auto-advance discussion note to the real decider (not 'system/auto-advance') | 6/6 |
...
| P3198 | issue: long-running services hang on SIGTERM — stop only completes via systemd timeout kill (regression vs P266/P267) | 3/4 |
