...
-- Knowledge base: kbDocument, kbEmbedding, kbTag.
-- Run with: psql -v schema_name=agentHive -f 006-kb.sql
-- Requires: pgvector extension for kbEmbedding.vector column.
-- ============================================================
...
-- ============================================================
-- kbDocument — knowledge base documents
-- ============================================================
CREATE TABLE IF NOT EXISTS :schema_name.kbDocument (
...
CREATE INDEX IF NOT EXISTS kbdoc_source ON :schema_name.kbDocument (source_type, source_ref);
CREATE INDEX IF NOT EXISTS kbdoc_active ON :schema_name.kbDocument (slug)
  WHERE lifecycle_status = 'active';

CREATE OR REPLACE TRIGGER set_updated_at_kbdocument
  BEFORE UPDATE ON :schema_name.kbDocument
  FOR EACH ROW EXECUTE FUNCTION :schema_name.set_updated_at();

COMMENT ON TABLE :schema_name.kbDocument IS
  'Knowledge base documents. One row per chunk for large documents (chunk_index tracks order).';

-- ============================================================
-- kbEmbedding — vector embeddings for semantic search
-- ============================================================
CREATE TABLE IF NOT EXISTS :schema_name.kbEmbedding (
  id               BIGINT       GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  document_id      BIGINT       NOT NULL REFERENCES :schema_name.kbDocument (id) ON DELETE CASCADE,
...
-- ============================================================
CREATE TABLE IF NOT EXISTS :schema_name.kbTag (
  document_id      BIGINT       NOT NULL REFERENCES :schema_name.kbDocument (id) ON DELETE CASCADE,
  tag              TEXT         NOT NULL,
  PRIMARY KEY (document_id, tag)
);

CREATE INDEX IF NOT EXISTS kbtag_tag ON :schema_name.kbTag (tag);

COMMENT ON TABLE :schema_name.kbTag IS
