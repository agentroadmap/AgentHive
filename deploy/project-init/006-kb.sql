...
-- Created immediately during project-init (safe for empty tables, builds incrementally as data is added).
-- pgvector 0.8.2+ supports HNSW, which doesn't require rebuilds after bulk loads unlike IVFFlat.
CREATE INDEX IF NOT EXISTS kb_embedding_hnsw ON :schema_name.kb_embedding USING hnsw (vector vector_cosine_ops);
...
COMMENT ON TABLE :schema_name.kb_embedding IS
  ...
  'HNSW index created at project-init time and builds incrementally. No rebuild needed after bulk loads.';
...
CREATE INDEX IF NOT EXISTS kb_tag_tag ON :schema_name.kb_tag (tag);
