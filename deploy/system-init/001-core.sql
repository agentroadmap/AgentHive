...
COMMENT ON TABLE core.host IS
  'Hosts registered with this installation. Referenced by core.os_user and agency tables.';

-- ============================================================
-- core.os_user — OS-level users running AgentHive processes
-- ============================================================
...
COMMENT ON TABLE core.os_user IS
  'OS users on registered hosts. Agencies bind to (host, os_user) so the exact Linux user per agency process is known.';
...
-- ============================================================
-- core.runtime_flag — DB-driven feature flags
-- ============================================================
...
-- ============================================================
-- core.service_heartbeat — service liveness signal
-- ============================================================
...
-- ============================================================
-- core.runtime_endpoint — canonical service endpoint registry
-- ============================================================
CREATE TABLE IF NOT EXISTS core.runtime_endpoint (
  id               BIGINT       GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS runtime_endpoint_active
  ON core.runtime_endpoint (service_key)
  WHERE lifecycle_status = 'active';
...
CREATE OR REPLACE TRIGGER runtime_endpoint_change_notify
  AFTER INSERT OR UPDATE OR DELETE ON core.runtime_endpoint
...
CREATE OR REPLACE TRIGGER set_updated_at_runtime_endpoint
  BEFORE UPDATE ON core.runtime_endpoint
  FOR EACH ROW EXECUTE FUNCTION core.set_updated_at();

COMMENT ON TABLE core.runtime_endpoint IS
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'agenthive_orchestrator') THEN
    ...
    GRANT SELECT, INSERT, UPDATE ON core.runtime_endpoint TO agenthive_orchestrator;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'agenthive_agency') THEN
    ...
    GRANT SELECT ON core.runtime_flag, core.host, core.os_user, core.runtime_endpoint, core.project TO agenthive_agency;
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'agenthive_a2a') THEN
    ...
    GRANT SELECT ON core.runtime_flag, core.runtime_endpoint TO agenthive_a2a;
...
INSERT INTO core.runtime_endpoint (service_key, url, protocol, owner_did)
VALUES
