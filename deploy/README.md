...
| `kb_embedding` | Vector embeddings — `vector(1536)` column (OpenAI default); unique on (document_id, model_id); IVFFlat index added post-load |
...
**Field-level version history** — The `trg_pVersion` trigger on `proposal` captures a JSONB delta of every changed field into `p_version`, with actor tracking via `app.current_actor`.
