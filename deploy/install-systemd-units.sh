...
#
# This script installs .service and .timer files from scripts/systemd/ to the
# system's /etc/systemd/system directory, then reloads the daemon and enables
...
UNITS_DIR="scripts/systemd"

if [[ ! -d "$UNITS_DIR" ]]; then
	echo "Error: $UNITS_DIR not found"
	exit 1
fi
