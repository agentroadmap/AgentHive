...
/**
 * P933 AC-6: formatAgentLabelLive(row, hostIds: string[]).
 * hostIds is caller-supplied (DB query result); the function never queries the DB.
 * Mirrors the SQL CASE in roadmap_workforce.v_agent_display_label.
 */
...
describe("formatAgentLabelLive", () => {
	...
	test("TC-7: empty hostIds disables stripping (alias shown verbatim with identity)", () => {
		const result = formatAgentLabelLive(
			{ display_alias: "Claude-Bot-Documenter", agent_identity: "ccs46ant-bot-docum-a" },
			[],
		);
