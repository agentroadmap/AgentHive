 ...
 *
 * Hot-configurable via DISPATCH_LOOP_THRESHOLD_PER_HOUR runtime flag; falls
 * back to the legacy AGENTHIVE_DISPATCH_LOOP_THRESHOLD env, then the
 * historical default of 6.
 */
async function resolveDispatchLoopThreshold(): Promise<number> {
	try {
		...
		const n = Number(process.env.AGENTHIVE_DISPATCH_LOOP_THRESHOLD ?? "6");
		return Number.isFinite(n) ? n : 6;
	}
}

/**
 * P1729: Max accumulated blocking reviews before convergence guard pauses.
 * Hot-configurable via GATE_CONVERGENCE_MAX_BLOCKING runtime flag.
 */
async function resolveGateConvergenceMaxBlocking(): Promise<number> {
	try {
		...
		const n = Number(process.env.AGENTHIVE_GATE_CONVERGENCE_MAX_BLOCKING ?? "3");
		return Number.isFinite(n) ? n : 3;
	}
}

/**
 * P1729: Max per-role dispatch runs before convergence guard pauses.
 * Hot-configurable via GATE_CONVERGENCE_MAX_RUNS_PER_ROLE runtime flag.
 */
async function resolveGateConvergenceMaxRunsPerRole(): Promise<number> {
	try {
		...
		const n = Number(process.env.AGENTHIVE_GATE_CONVERGENCE_MAX_RUNS_PER_ROLE ?? "8");
...
async function checkConvergenceGuard(
	...
	const maxBlocking = await resolveGateConvergenceMaxBlocking();
	const maxRunsPerRole = await resolveGateConvergenceMaxRunsPerRole();
...
async function postWorkOfferImpl(
	...
	const dispatchLoopThreshold = await resolveDispatchLoopThreshold();
	...
	if (recentRuns > dispatchLoopThreshold) {
		await queryFn(
			`UPDATE roadmap_proposal.proposal
			    SET gate_scanner_paused = true,
		...
		await queryFn(
			...
			[
				...
				`postWorkOffer refused: ${recentRuns} completed/failed runs for role "${input.role}" in last 1h (threshold ${dispatchLoopThreshold}). gate_scanner_paused=true. Investigate why the runs are not advancing state/maturity.`,
				JSON.stringify({
					proposal_id: input.proposalId,
					role: input.role,
		...
		throw new DispatchLoopError(input.proposalId, input.role, recentRuns, dispatchLoopThreshold);
	}
