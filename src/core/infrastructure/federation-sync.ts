...
// P3787: these constants are now hot-configurable via core.runtime_flag.
async function resolveFederationSyncPollIntervalMs(): Promise<number> {
	try { return await runtimeConfig.get(FlagKeys.FEDERATION_SYNC_POLL_INTERVAL_MS); } catch { return 30_000; }
}
async function resolveFederationHealthQuarantineThreshold(): Promise<number> {
	try { return await runtimeConfig.get(FlagKeys.FEDERATION_HEALTH_QUARANTINE_THRESHOLD); } catch { return 3; }
}
async function resolveFederationPingTimeoutMs(): Promise<number> {
	try { return await runtimeConfig.get(FlagKeys.FEDERATION_PING_TIMEOUT_MS); } catch { return 5_000; }
}
...
export class FederationSync extends EventEmitter {
	private readonly config: Omit<FederationConfig, "queryFn">;
	...
	async checkPeerHealth(peer: PeerRecord): Promise<boolean> {
		...
		if (healthy) {
			...
			const quarantineThreshold = await resolveFederationHealthQuarantineThreshold();
	...
	async start(): Promise<void> {
		this.stopped = false;
		const intervalMs = this.config.pollIntervalMs ?? await resolveFederationSyncPollIntervalMs();
