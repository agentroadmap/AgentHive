...
async function resolveGatewayWakeTimeoutMs(): Promise<number> {
  try { return await runtimeConfig.get(FlagKeys.GATEWAY_WAKE_TIMEOUT_MS); }
  catch { return DEFAULT_WAKE_TIMEOUT_MS; }
}
  ...
  buildAdapter(row: RegistryRow): TransportAdapter {
    ...
    return {
      ...
      wakeUp: async (wakeTimeoutMs?: number): Promise<void> => {
        const resolvedTimeout = wakeTimeoutMs ?? await resolveGatewayWakeTimeoutMs();
        await pool.query(
