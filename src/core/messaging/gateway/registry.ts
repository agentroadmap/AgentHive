...
async function resolveGatewayWakeTimeoutMs(): Promise<number> {
  try {
    return await runtimeConfig.get(FlagKeys.GATEWAY_WAKE_TIMEOUT_MS);
  } catch {
  ...
  buildAdapter(row: RegistryRow): TransportAdapter {
    ...
    return {
      ...
      wakeUp: async (wakeTimeoutMs?: number): Promise<void> => {
        const timeout = wakeTimeoutMs ?? await resolveGatewayWakeTimeoutMs();
        await pool.query(
        ...
        throw new TransportWakeTimeoutError(transportId, timeout);
      },
