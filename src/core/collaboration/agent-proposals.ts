...
/** P3787: resolve AGENT_PROPOSAL_LEASE_TTL_MS from runtime_flag with fallback. */
export async function resolveAgentProposalLeaseTtlMs(): Promise<number> {
	try { return await runtimeConfig.get(FlagKeys.AGENT_PROPOSAL_LEASE_TTL_MS); } catch { return DEFAULT_LEASE_TTL_MS; }
}
