...
import {
	...
	listenForMessages,
	receiveLiaisonPong,
} from "./liaison-message-service.ts";
...
export async function sendDirectiveToAgent(
	...
	// P1099 AC-1: Canonicalize agentIdentity before INSERT
	let canonicalAgentIdentity: string;
	try {
		canonicalAgentIdentity = canonicalizeIdentity(agentIdentity);
	} catch (err) {
		if (err instanceof InvalidIdentityError) {
			throw new Error(`Invalid agentIdentity (P1099): ${err.message}`);
		}
		throw err;
	}
...
export async function broadcastToHiveCentral(
	...
	// P1099 AC-1: Canonicalize fromAgencyId before INSERT
	let canonicalFromAgencyId: string;
	try {
		canonicalFromAgencyId = canonicalizeIdentity(fromAgencyId);
	} catch (err) {
		if (err instanceof InvalidIdentityError) {
			throw new Error(`Invalid fromAgencyId (P1099): ${err.message}`);
		}
		throw err;
	}
	...
	switch (msg.kind) {
		case "assistance_request": {
			...
			if (action.type === "escalate") {
				await broadcastToHiveCentral(
					agencyId,
					`Assistance escalated: ${action.description}`,
					{
		...
		case "liaison_pong":
			await receiveLiaisonPong(
				agencyId,
				String((msg.payload as any).nonce ?? ""),
			);
			break;
		...
		case "task_error":
			...
			if (process.env.DEBUG_LIAISON_HUB) {
				console.log(
					`[LiaisonHub] ${agencyId}: worker report kind='${msg.kind}'`,
				);
			}
		...
		default:
			if (process.env.DEBUG_LIAISON_HUB) {
				console.log(
					`[LiaisonHub] ${agencyId}: unhandled kind='${msg.kind}' seq=${msg.sequence}`,
				);
			}
	}
}
	...
	const run = async () => {
		...
		try {
			const replayWindowHours = parseInt(
				process.env.LIAISON_REPLAY_WINDOW_HOURS ?? "6",
				10,
			);
			const createdAfter = new Date(
				Date.now() - replayWindowHours * 60 * 60 * 1000,
			);
			const unacked = await getUnackedMessages(
				agencyId,
				undefined,
				createdAfter,
			);
			if (unacked.length > 0) {
				console.log(
					`[LiaisonHub] ${agencyId}: replaying ${unacked.length} unacked message(s) (window=${replayWindowHours}h)`,
				);
				for (const msg of unacked) {
					...
					try {
						...
						console.error(
							`[LiaisonHub] ${agencyId}: replay error kind='${msg.kind}':`,
							err,
						);
		...
		try {
			for await (const msg of listenForMessages(agencyId, controller.signal)) {
				...
				try {
					...
					console.error(
						`[LiaisonHub] ${agencyId}: dispatch error kind='${msg.kind}':`,
						err,
					);
					await acknowledgeMessage(msg.message_id, "reject", String(err)).catch(
						() => undefined,
					);
				}
			}
