...
export async function runLiaisonAgent(
	...
	const rawAllowlist = await runtimeConfig
		.get(FlagKeys.SELF_CLAIM_AGENCIES)
		.catch(() => process.env.AGENTHIVE_SELF_CLAIM_AGENCIES ?? "");
	const selfClaimAllowlist = rawAllowlist
		.split(",")
		.map((s: string) => s.trim())
		.filter(Boolean);
	const allowedByList =
