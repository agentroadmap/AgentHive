...
async function runChecks(ctx: HiveContext): Promise<DoctorCheck[]> {
	...
	checks.push({
		...
		remediation: ctx.project
			? undefined
			: "Run `hive init` to initialize a project, or set HIVE_PROJECT env.",
	});
	...
	checks.push({
		...
		remediation: ctx.agency
			? undefined
			: "Set HIVE_AGENCY=<agency_id> in your environment.",
	});
	...
	checks.push({
		...
		remediation: dbOk
			? undefined
			: "Check postgres service: `sudo systemctl status postgresql`",
	});
	...
	checks.push({
		...
		severity: "ok",
		message:
			"Budget check skipped (requires DB read; run `hive budget show` for details)",
	});
...
export interface TopologyProbers {
	...
	poolQuery?: (
		...
		rows: Array<{ agent_identity: string; is_attached: boolean }>;
	}>;
...
async function probeHealthDefault(
	url: string,
	timeoutMs: number,
): Promise<{ ok: boolean; latencyMs: number; error?: string }> {
	...
	try {
		...
		if (body.status !== "ok") {
			return {
				ok: false,
				latencyMs,
				error: `status field is "${body.status}"`,
			};
		}
...
export async function checkTopology(
	ctx: HiveContext,
	probers: TopologyProbers = {},
): Promise<DoctorCheck> {
	const execFn =
		probers.execSync ?? (await import("node:child_process")).execSync;
	const queryFn =
		probers.poolQuery ??
		((sql: string, params: unknown[]) =>
			getPool().query<{ agent_identity: string; is_attached: boolean }>(
				sql,
				params,
			));
	...
	const expectedAgencies: string[] = [];
	const unattachedAgencies: string[] = [];
	try {
	...
	if (dataSourceErrors.length === 0 && unattachedAgencies.length > 0) {
		...
		if (unattachedAgencies.length <= 2) {
			...
			remediation =
				"Verify a2a-host is running and all agencies have active LISTEN sessions.";
		}
	}
...
export function registerDoctor(
	...
	program
		.command("doctor")
		.description(
			"Run system readiness checks (14 checks, severity + remediation per check)",
		)
		.option("--project <P>", "Project slug override")
		.option(
			"--check <NAME>",
			"Run only checks whose name contains NAME (substring match)",
		)
		...
		.option(
			"-o, --format <FMT>",
			"Output format (text|json|jsonl|yaml)",
			"text",
		)
		...
		.action(async (opts) => {
			...
			if (!opts.quiet) {
				if (fmt === "text") {
					...
					printText([""]);
					if (hasErrors)
						printText(["Status: ERRORS — see [ERROR] checks above."]);
					else if (hasWarnings)
						printText(["Status: WARNINGS — system functional with caveats."]);
