...
const DependencyInput: React.FC<DependencyInputProps> = ({
	...
	useEffect(() => {
		if (inputValue.trim()) {
			const filtered = availableProposals.filter(
				(proposal) =>
			...
			<div className="relative w-full">
				...
				>
					...
					<textarea
						...
						placeholder={
							value.length === 0
								? "Type proposal ID or title, then press Enter or comma"
					...
					<div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg max-h-64 overflow-auto overscroll-contain transition-colors duration-200">
						{suggestions.map((proposal, index) => (
							...
							>
								...
								<div className="text-gray-600 dark:text-gray-300 break-words whitespace-normal">
									{proposal.title}
								</div>
							</button>
						))}
