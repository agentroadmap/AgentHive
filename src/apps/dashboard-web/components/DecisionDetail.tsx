...
export default function DecisionDetail({
	...
	const [originalDecisionTitle, setOriginalDecisionTitle] =
		useState<string>("");
	...
	useEffect(() => {
		if (id === "new") {
			...
			void loadDecisionContent();
		}
	}, [id, loadDecisionContent]);
	...
	const hasChanges =
		content !== originalContent || decisionTitle !== originalDecisionTitle;
			...
			<div className="flex-1 flex items-center justify-center p-8">
				<div className="text-center">
					...
					<p className="mt-1 text-sm text-gray-500">
						Select a decision from the sidebar to view its content.
					</p>
				</div>
			</div>
		);
							...
							<div className="flex-1">
								{isEditing ? (
									...
									<h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2 transition-colors duration-200">
										{decisionTitle ||
											decision?.title ||
											(title ? decodeURIComponent(title) : `Decision ${id}`)}
									</h1>
								)}
