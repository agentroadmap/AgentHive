...
const Settings: React.FC = () => {
	...
	const [originalConfig, setOriginalConfig] = useState<RoadmapConfig | null>(
		null,
	);
	...
	const validateConfig = (): boolean => {
		...
		if (
			config.defaultPort &&
			(config.defaultPort < 1 || config.defaultPort > 65535)
		) {
				...
				<div className="space-y-8">
						...
						<div className="space-y-4">
								...
								<label className="flex items-center justify-between">
									<div>
										...
										<p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
											Automatically commit changes to Git after proposal
											operations
										</p>
									</div>
									<div className="relative inline-flex items-center cursor-pointer">
										<input
											...
											onChange={(e) =>
												handleInputChange("autoCommit", e.target.checked)
											}
								...
								<label className="flex items-center justify-between">
									...
									<div className="relative inline-flex items-center cursor-pointer">
										<input
											...
											onChange={(e) =>
												handleInputChange("remoteOperations", e.target.checked)
											}
											className="sr-only peer"
										/>
										<div className="w-11 h-6 bg-gray-200 dark:bg-gray-700 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-circle peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-circle after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-500"></div>
							...
							<div>
								...
								<select
									...
									onChange={(e) =>
										handleInputChange("defaultStatus", e.target.value)
									}
									className="w-full h-10 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-stone-500 dark:focus:ring-stone-400 transition-colors duration-200"
								>
									{statuses.map((status) => (
							...
							<div>
								...
								<input
									...
									onChange={(e) =>
										handleInputChange("defaultEditor", e.target.value)
									}
								...
								/>
								<p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
									Editor command to use for editing proposals (overrides EDITOR
							...
							<div>
								...
								<input
									...
									onChange={(e) =>
										handleInputChange(
											"defaultPort",
											parseInt(e.target.value, 10) || 6420,
										)
									}
									className={`w-full px-3 py-2 border rounded-lg text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-stone-500 dark:focus:ring-stone-400 transition-colors duration-200 ${
								...
								<label className="flex items-center justify-between">
									...
									<div className="relative inline-flex items-center cursor-pointer">
										<input
											...
											onChange={(e) =>
												handleInputChange("autoOpenBrowser", e.target.checked)
											}
											className="sr-only peer"
										/>
										<div className="w-11 h-6 bg-gray-200 dark:bg-gray-700 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-circle peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-circle after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-500"></div>
					...
					<div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
						...
						<div className="space-y-4">
							...
							<div>
								<label
									htmlFor="proposalResolutionStrategy"
									className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
								>
								...
								<select
									...
									onChange={(e) =>
										handleInputChange(
											...
											e.target.value as "most_recent" | "most_progressed",
										)
									}
									className="w-full h-10 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-stone-500 dark:focus:ring-stone-400 transition-colors duration-200"
								>
								...
								<p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
									Strategy for resolving conflicts when proposals exist in
									multiple branches
								</p>
							</div>
							...
							<div>
								...
								/>
								<p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
									Number of digits for ID padding (0 = disabled, 3 =
									proposal-001, 4 = proposal-0001)
								</p>
							</div>
							...
							<div>
								...
								/>
								<p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
									Set during initialization. Cannot be changed to avoid breaking
