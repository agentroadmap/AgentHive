...
export const Checkbox = React.forwardRef<HTMLInputElement, CheckboxProps>(
  ({ label, indeterminate = false, error, hint, className, id, disabled, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        >
          ...
          />
          {label && (
            <span className="text-sm text-[var(--color-text)] leading-5">{label}</span>
          )}
        </label>
        {error && (
          <p id={errorId} role="alert" className="text-xs text-[var(--color-error)] ml-6">
            {error}
          </p>
        )}
        {hint && !error && (
          <p id={hintId} className="text-xs text-[var(--color-text-muted)] ml-6">
            {hint}
          </p>
        )}
      </div>
