...
const AcceptanceCriteriaEditor: React.FC<Props> = ({
	...
	const handleRemove = (index: number) => {
		...
		const updated = preserveIndices
			? filtered
			: filtered.map((c, i) => ({ ...c, index: i + 1 }));
		setCriteria(updated);
		onChange(updated);
	};
	...
	return (
		<div className="space-y-2">
			<div className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 transition-colors duration-200">
				{label}
			</div>
