...
export async function recordGateDecision(args: {
	...
	// Operator token verified via the P3508 authorizeOperatorByToken path (NOT the
	// impersonable app.agent_identity GUC). Used by P3565 diff-risk (high-risk merge
	// authorization) AND P1391 (the ELEVATED reject verdict, human-operator path).
	operator_token?: string;
	// P3565 diff-risk: diff signal sources for the high-risk advance check.
